  ...
  return (
    <div className="min-h-screen flex flex-col md:grid md:grid-cols-2" style={{ fontFamily: "'Hanken Grotesk', sans-serif" }}>
      ...
      <div className="md:hidden flex flex-col items-center pt-10 pb-6 px-6"
        ...
        <h1 style={{ fontFamily: "'Bricolage Grotesque', sans-serif", fontWeight: 800, fontSize: 24, color: '#fff', margin: 0 }}>Create your account</h1>
        <p style={{ color: 'rgba(255,255,255,0.82)', fontSize: 14, marginTop: 6, textAlign: 'center' }}>Start booking grooming in under a minute.</p>
      </div>
      ...
      <div className="bg-[#FDFAF4] flex flex-col justify-center px-6 md:px-12 py-8 md:py-12">
        ...
        <p className="hidden md:block text-[13.5px] text-[#8C7F76] mb-7">Start booking grooming in under a minute.</p>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          <div>
            <label className="block text-[12.5px] font-semibold text-[#3E342E] mb-1.5">Full name</label>
            <input type="text" value={name} onChange={e => setName(e.target.value)} placeholder="Jamie Rivera" required
          ...
          <button type="submit" disabled={loading}
            ...
            {loading ? 'Creating account…' : <><span>Create account</span><Icon name="arrow" size={17} color="#fff" /></>}
          </button>
        </form>

        <p className="text-[13px] text-[#8C7F76] text-center mt-6">
          Already have an account?{' '}
          <Link to="/login" className="text-[#E87E5B] font-bold" style={{ textDecoration: 'none' }}>Log in</Link>
        </p>
      </div>
